...
class TestInMemoryMilestoneRepository:
    ...
    @pytest.mark.anyio
    async def test_マイルストーンを保存して取得できること(
        self, repository, sample_milestone
    ):
        """マイルストーンを保存してIDで取得できること"""
        # 保存
        result = await repository.save(sample_milestone)
        assert result.is_ok()

        # 取得
        found_result = await repository.find_by_id(sample_milestone.id)
    ...
    @pytest.mark.anyio
    async def test_ユーザーidでマイルストーンを検索できること(
        self, repository, sample_milestone
    ):
        """ユーザーIDでマイルストーンを検索できること"""
        # 保存
        await repository.save(sample_milestone)

        # ユーザーIDで検索
        found_result = await repository.find_by_user_id(sample_milestone.user_id)
        assert found_result.is_ok()
    ...
    @pytest.mark.anyio
    async def test_複数のマイルストーンをユーザーidで検索できること(self, repository):
        ...
        # ユーザーIDで検索
        found_result = await repository.find_by_user_id(user_id)
        assert found_result.is_ok()
        milestones = found_result.unwrap()
        assert len(milestones) == 2

    @pytest.mark.anyio
    async def test_存在しないマイルストーンのid検索はerrを返すこと(self, repository):
    ...
    @pytest.mark.anyio
    async def test_マイルストーンを削除できること(self, repository, sample_milestone):
        """マイルストーンを削除できること"""
        # 保存
        await repository.save(sample_milestone)

        # 削除
        delete_result = await repository.delete(sample_milestone.id)
    ...
    @pytest.mark.anyio
    async def test_存在しないマイルストーンの削除はerrを返すこと(self, repository):
    ...
    @pytest.mark.anyio
    async def test_マイルストーンを更新できること(self, repository, sample_milestone):
        ...
        await repository.save(updated_milestone)
