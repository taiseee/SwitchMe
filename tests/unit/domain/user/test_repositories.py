...
class TestInMemoryUserRepository:
    ...
    @pytest.mark.anyio
    async def test_ユーザーを保存して取得できること(self, repository, sample_user):
        """ユーザーを保存してIDで取得できること"""
        # 保存
        ...
        # 取得
        found_result = await repository.find_by_id(sample_user.id)
    ...
    @pytest.mark.anyio
    async def test_メールアドレスでユーザーを検索できること(
        ...
        # メールアドレスで検索
        found_result = await repository.find_by_email(sample_user.email)
        assert found_result.is_ok()
        found_user = found_result.unwrap()
        assert found_user.id == sample_user.id
        assert found_user.email == sample_user.email
    ...
    @pytest.mark.anyio
    async def test_OAuthプロバイダーとユーザーIDでユーザーを検索できること(
        self, repository, sample_user
    ):
        ...
        # OAuthで検索
        found_result = await repository.find_by_oauth(
    ...
    @pytest.mark.anyio
    async def test_存在しないユーザーのid検索はerrを返すこと(self, repository):
    ...
    @pytest.mark.anyio
    async def test_存在しないユーザーのemail検索はerrを返すこと(self, repository):
    ...
    @pytest.mark.anyio
    async def test_存在しないユーザーのOAuth検索はerrを返すこと(self, repository):
        """存在しないユーザーのOAuth検索はErrを返すこと"""
        result = await repository.find_by_oauth(
            oauth_provider=OAuthProvider(value="google"),
    ...
    @pytest.mark.anyio
    async def test_ユーザーを削除できること(self, repository, sample_user):
        """ユーザーを削除できること"""
        # 保存
        await repository.save(sample_user)

        # 削除
        delete_result = await repository.delete(sample_user.id)
    ...
    @pytest.mark.anyio
    async def test_存在しないユーザーの削除はerrを返すこと(self, repository):
    ...
    @pytest.mark.anyio
    async def test_ユーザーを更新できること(self, repository, sample_user):
